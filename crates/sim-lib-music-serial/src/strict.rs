//! Strict realization of immutable serial plans.

use std::cmp::Reverse;
use std::collections::{BTreeMap, BTreeSet, BinaryHeap};

use sim_lib_music_core::{Articulation, Channel, Note, Pitch, Time};

use crate::{
    RealizedSerialEvent, RealizedSerialNote, RealizedSerialOrigin, SerialEventId, SerialPlan,
    SerialRealization, StrictRealizationError,
};
...
/// Complete explicit choices required to realize one strict serial plan.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct StrictRealizationContext {
    /// One explicit spec per planned event.
...
pub fn realize_strict(
    plan: &SerialPlan,
    context: &StrictRealizationContext,
    ...
    for unit_idx in order {
        ...
        for event_id in &unit.members {
            ...
            if matches!(spec.sound, EventSound::Notes) {
            ...
            events.insert(
                ...
                RealizedEventState {
                    event: RealizedSerialEvent {
                        ...
                        is_rest: matches!(spec.sound, EventSound::Rest),
        ...
        cursor += match context.simultaneous_policy {
            SimultaneousRenderPolicy::PreserveOnset => unit_duration,
        };
    }
    ...
    Ok(SerialRealization::new(
        plan.clone(),
        events.into_values().map(|state| state.event).collect(),
        notes,
    ))
}

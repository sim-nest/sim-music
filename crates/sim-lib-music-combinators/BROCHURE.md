...
- Rotate, reflect, slice, and overlay sparse or rectangular music carpets under explicit boundary and collision policy.
- Address every carpet cell through the shared mixed-radix rank space.
- Round-trip retained relative origins exactly and receive named loss evidence when an origin or source structure is omitted.
- Grow inspectable musical grammars only within declared generation and expansion limits.
- Rank finite progression trees and filter their catalog through shared deterministic search.
- Follow scale changes through explicit pitch maps instead of buried modulation branches.

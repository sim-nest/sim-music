...
use crate::{
    AdsrEnvelope, AdsrSettings, ComponentRegistry, Lfo, LfoSettings, Oscillator, OscillatorKind,
    PhaseOscillator,
};
...
fn output_ticks(config: &ModulatorConfig, cx: &PlayContext) -> Vec<Tick> {
...
impl ModulatorGenerator {
    fn new(config: &ModulatorConfig, cx: &PlayContext) -> Self {
        let mut lfo = None;
...
fn apply_operator(samples: &mut [ModulatorSample], operator: &ModulationOperator) {
    match *operator {
...
fn scaled_control(value: f32) -> i64 {
...
fn modulator_hash(config: &ModulatorConfig, events: &[PlayEvent], seed: u64) -> String {
